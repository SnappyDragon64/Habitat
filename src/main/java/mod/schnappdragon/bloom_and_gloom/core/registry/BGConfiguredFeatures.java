...
public class BGConfiguredFeatures {
    ...
    public static final ConfiguredFeature<?, ?> HUGE_FAIRY_RING_MUSHROOM = BGFeatures.HUGE_FAIRY_RING_MUSHROOM_FEATURE.get().withConfiguration(new BigMushroomFeatureConfig(new SimpleBlockStateProvider(BGBlocks.FAIRY_RING_MUSHROOM_BLOCK.get().getDefaultState().with(HugeMushroomBlock.DOWN, false)), new SimpleBlockStateProvider(BGBlocks.FAIRY_RING_MUSHROOM_STEM.get().getDefaultState().with(HugeMushroomBlock.UP, false).with(HugeMushroomBlock.DOWN, false)), 2)).withPlacement(Features.Placements.HEIGHTMAP_PLACEMENT).chance(40);
