...
public class HabitatConfiguredFeatures {
    //public static final ConfiguredFeature<?, ?> PATCH_RAFFLESIA = Feature.RANDOM_PATCH.configured(FeatureUtils.simplePatchConfiguration(BlockStateProvider.simple(HabitatBlocks.RAFFLESIA.get().defaultBlockState()), SimpleBlockPlacer.INSTANCE).xspread(6).yspread(1).zspread(6).tries(2).whitelist(ImmutableSet.of(Blocks.GRASS_BLOCK)).build()).decorated(Features.Decorators.HEIGHTMAP_SQUARE).rarity(4);

    //public static final ConfiguredFeature<?, ?> PATCH_KABLOOM_BUSH = Feature.RANDOM_PATCH.configured(FeatureUtils.simplePatchConfiguration(BlockStateProvider.simple(HabitatBlocks.KABLOOM_BUSH.get().defaultBlockState().setValue(KabloomBushBlock.AGE, 7)), SimpleBlockPlacer.INSTANCE).xspread(4).yspread(1).zspread(4).tries(20).whitelist(ImmutableSet.of(Blocks.GRASS_BLOCK)).build()).decorated(Features.Decorators.HEIGHTMAP_SQUARE).rarity(160);

    //public static final ConfiguredFeature<?, ?> PATCH_SLIME_FERN = HabitatFeatures.SLIME_FERN_FEATURE.get().configured(FeatureUtils.simplePatchConfiguration(BlockStateProvider.simple(HabitatBlocks.SLIME_FERN.get().defaultBlockState()), SimpleBlockPlacer.INSTANCE).xspread(4).yspread(5).zspread(4).tries(50).build()).rangeUniform(VerticalAnchor.bottom(), VerticalAnchor.absolute(40)).rarity(2);

    //public static final ConfiguredFeature<?, ?> PATCH_BALL_CACTUS = Feature.RANDOM_PATCH.configured(FeatureUtils.simplePatchConfiguration(new WeightedStateProvider(SimpleWeightedRandomList.<BlockState>builder().add(HabitatBlocks.FLOWERING_ORANGE_BALL_CACTUS.get().defaultBlockState(), 3).add(HabitatBlocks.FLOWERING_PINK_BALL_CACTUS.get().defaultBlockState(), 3).add(HabitatBlocks.FLOWERING_RED_BALL_CACTUS.get().defaultBlockState(), 2).add(HabitatBlocks.FLOWERING_YELLOW_BALL_CACTUS.get().defaultBlockState(), 1)), SimpleBlockPlacer.INSTANCE).xspread(5).yspread(1).zspread(5).tries(5).build()).decorated(Features.Decorators.HEIGHTMAP_SQUARE).rarity(16);
