...
public class FairyRingMushroomStewItem extends Item {
    ...
    public ItemStack onItemUseFinish(ItemStack stack, World worldIn, LivingEntity entityLiving) {
        ...
        for (int i = 0; i < 4; ++i)
            worldIn.addParticle(HabitatParticleTypes.FAIRY_RING_SPORE.get(), entityLiving.getPosX() + entityLiving.getRNG().nextDouble() / 2.0D, entityLiving.getPosYHeight(0.5D), entityLiving.getPosZ() + entityLiving.getRNG().nextDouble() / 2.0D, entityLiving.getRNG().nextGaussian() * 0.01D, 0.0D, entityLiving.getRNG().nextGaussian() * 0.01D);
